...
/****** Object:  StoredProcedure [dbo].[usp_validar_estudiante_exista_por_id_interno]    Script Date: 30/07/2026 8:42:33 p. m. ******/
DROP PROCEDURE [dbo].[usp_validar_estudiante_exista_por_id_interno]
GO
...
CREATE       PROCEDURE [dbo].[usp_validar_estudiante_exista_por_id_interno]  
(  
...
BEGIN  
    ...
    BEGIN CATCH  
        SELECT @mensajeUsuarioResultado = 'Ocurrió un error inesperado.',   
               @mensajeTecnicoResultado = CONCAT('Error crítico en orquestador [usp_validar_estudiante_exista_por_id_interno]: ', ERROR_MESSAGE(), '. Línea: ', ERROR_LINE()),
               @estadoResultado = 0;  
    END CATCH  
END
GO
