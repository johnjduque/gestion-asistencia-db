...
CREATE OR ALTER PROCEDURE [dbo].[usp_crear_coordinador_interno]
(
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- PASO 2: Validar existencia del programa
        IF @estadoResultado = 1
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM dbo.Programa WHERE id = @idProgramaDefecto)
            BEGIN
        ...
        -- PASO 5: Registro atómico transaccional de Usuario, Coordinador y asignación a Programa
        IF @estadoResultado = 1
        BEGIN
            ...
            VALUES (@idCoordinadorDefecto, @idUsuarioNuevo);
