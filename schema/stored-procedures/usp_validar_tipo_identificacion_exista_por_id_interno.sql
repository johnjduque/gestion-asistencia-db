...
AS
    ...
    DECLARE @idTipoIdDefecto      UNIQUEIDENTIFIER = dbo.ufn_obtener_parametro_guid(@idTipoId, 'GENERAL', 'GUID_DEFECTO_CORRELACION');
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @estadoResultado = 1 AND @idTipoIdDefecto = TRY_CAST(dbo.ufn_obtener_parametro('GENERAL', 'GUID_DEFECTO_CORRELACION') AS UNIQUEIDENTIFIER)
        BEGIN
        ...
        -- PASO 3: Validación de existencia en la vista uv_tipo_identificacion
        IF @estadoResultado = 1 AND NOT EXISTS (SELECT 1 FROM dbo.uv_tipo_identificacion WHERE id = @idTipoIdDefecto)
        BEGIN
            EXEC dbo.usp_obtener_mensaje_catalogo
                @p_codigo = 'GEN_001',
                @p_param1 = @idTipoIdDefecto,
                @mensajeUsuarioResultado = @mensajeUsuarioResultado OUTPUT,
                @mensajeTecnicoResultado = @mensajeTecnicoResultado OUTPUT;
