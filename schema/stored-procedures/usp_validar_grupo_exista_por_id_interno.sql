...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            SELECT @mensajeUsuarioResultado = 'El identificador del grupo no es v??lido.',
                   @mensajeTecnicoResultado = CONCAT('ID vac??o. ID CORRELACION=[', @idCorrelacionDefecto, ']'),
                   @estadoResultado = 0;
        END
        ...
        BEGIN
            SELECT  @idPeriodo = idPeriodoAcademico,
                    ...
                    @capacidadMaximaPermitida = capacidadMaximaPermitida, -- de la tabla Grupo v??a uv_grupo
            ...
            BEGIN
                SELECT  @mensajeUsuarioResultado = CONCAT('No existe un grupo con el identificador [', @idGrupoDefecto, '] o no cumple con los requisitos de la vista.'),
                        @mensajeTecnicoResultado = CONCAT('No se encontr?? el registro en uv_grupo. Nota: La vista usa INNER JOINs, verifique integridad de datos. ID CORRELACION=[', @idCorrelacionDefecto, ']'),
                        @estadoResultado = 0;
            END
        END

        -- 4. Validar Semestre Acad??mico
        IF @estadoResultado = 1 AND @idPeriodo IS NULL
        BEGIN
            SELECT  @mensajeUsuarioResultado = 'El grupo no tiene un periodo acad??mico v??lido asignado.',
                    @mensajeTecnicoResultado = CONCAT('El idPeriodoAcademicoGrupo es NULL en uv_grupo. ID CORRELACION=[', @idCorrelacionDefecto, ']'),
                    @estadoResultado = 0;
        END

        -- 5. Validar que el grupo est?? habilitado
        IF @estadoResultado = 1 AND @grupoEstaHablitado = 0
        BEGIN
            SELECT  @mensajeUsuarioResultado = 'El grupo seleccionado no se encuentra habilitado para el periodo acad??mico actual.',
        ...
        -- 6. Validar Tama??o M??ximo
        IF @estadoResultado = 1
        BEGIN
            IF @cuposRestantes <= 0
            BEGIN
                SELECT  @mensajeUsuarioResultado = 'El grupo ha superado el tama??o m??ximo de estudiantes permitido.',
                        @mensajeTecnicoResultado = [dbo].[ufn_obtener_mensaje_exito](@idCorrelacionDefecto, OBJECT_NAME(@@PROCID), CONCAT('Cupo lleno. Activos: ', ISNULL(@estudiantesActivos, 0), ' / Capacidad M??xima: ', ISNULL(@capacidadMaximaPermitida, 0))),
                        @estadoResultado = 0;
            END
        END
    ...
    BEGIN CATCH
        SELECT @mensajeUsuarioResultado = 'Error al validar que el grupo existe.',
               @mensajeTecnicoResultado = CONCAT('Error cr??tico en orquestador [usp_validar_grupo_exista_por_id_interno]: ', ERROR_MESSAGE(), '. L??nea: ', ERROR_LINE()),
               @estadoResultado = 0;
    END CATCH
END
GO
